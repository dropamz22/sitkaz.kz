...
// streak: { count, last, todayCount, freeze }
//   freeze — дата последней использованной «заморозки» (YYYY-MM-DD) или null.
// Заморозка прощает ОДИН пропущенный день, но не чаще раза в неделю: если
// человек вернулся после ровно одного пропуска и заморозка доступна — серия
// не обнуляется.
const FREEZE_COOLDOWN = 7; // дней между заморозками

// Целое число дней между двумя датами-строками YYYY-MM-DD (b − a)
function daysBetweenStr(a, b) {
  return Math.round((Date.parse(b) - Date.parse(a)) / 86400000);
}
const freezeReady = (st, today) => !st.freeze || daysBetweenStr(st.freeze, today) >= FREEZE_COOLDOWN;

export function registerActivity(st) {
  const today = todayStr();
  const s = st || { count: 0, last: null, todayCount: 0, freeze: null };
  if (s.last === today) return { ...s, todayCount: (s.todayCount || 0) + 1 };
  const gap = s.last ? daysBetweenStr(s.last, today) : Infinity;
  if (gap === 1) {
    return { ...s, count: (s.count || 0) + 1, last: today, todayCount: 1 };
  }
  if (gap === 2 && freezeReady(s, today)) {
    // пропущен ровно один день — заморозка спасает серию
    return { ...s, count: (s.count || 0) + 1, last: today, todayCount: 1, freeze: today };
  }
  return { ...s, count: 1, last: today, todayCount: 1 };
}

// Стрик для показа: держится сегодня/вчера, а также при одном пропуске,
// если ещё доступна заморозка; иначе обнуляется.
export function displayStreak(st) {
  if (!st || !st.last) return 0;
  const today = todayStr();
  const gap = daysBetweenStr(st.last, today);
  if (gap <= 1) return st.count || 0;
  if (gap === 2 && freezeReady(st, today)) return st.count || 0;
  return 0;
}

// Статус заморозки для интерфейса: готова ли и когда использовалась
export function freezeStatus(st) {
  if (!st) return { ready: true, last: null };
  return { ready: freezeReady(st, todayStr()), last: st.freeze || null };
}
...
// ── История активности по дням (для календаря) ──
// days: { "YYYY-MM-DD": число засчитанных действий за день }
export function bumpDay(days, n = 1) {
  const today = todayStr();
  return { ...(days || {}), [today]: (((days || {})[today]) || 0) + n };
}
