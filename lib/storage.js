...
// Мгновенный локальный прогресс — без сети. Нужен, чтобы приложение
// открывалось сразу, а облако/сервер подтягивались уже в фоне.
export function loadLocalFast(EMPTY) {
  const local = loadLocal();
  return local ? { ...EMPTY, ...local } : EMPTY;
}

// Ограничиваем ожидание сети: медленное облако не должно подвешивать вход.
const withTimeout = (promise, ms) =>
  Promise.race([promise, new Promise((r) => setTimeout(() => r(null), ms))]);

export async function loadProgress(EMPTY, { timeout = 2500 } = {}) {
  const local = loadLocal();
  ...
  const [server, cloud] = await Promise.all([
    serverAvailable() ? withTimeout(loadServer(), timeout) : null,
    cloudAvailable() ? withTimeout(loadCloud(), timeout) : null,
  ]);
